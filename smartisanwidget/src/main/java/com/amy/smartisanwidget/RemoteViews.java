...
public class RemoteViews implements Parcelable, Filter {
    ...
    private class SetDrawableParametersForSmartisanOs extends Action {

        public final static int TAG = 16;

        int colorFilter;
        PorterDuff.Mode filterMode;

        public SetDrawableParametersForSmartisanOs(int id, int colorFilter,
        ...
        public SetDrawableParametersForSmartisanOs(Parcel parcel) {
            viewId = parcel.readInt();
            colorFilter = parcel.readInt();
            boolean hasMode = parcel.readInt() != 0;
            if (hasMode) {
        ...
        public void apply(View root, ViewGroup rootParent, OnClickHandler handler) {
            ...
            targetDrawable = imageView.getDrawable();
            if (targetDrawable != null) {
                Object tag =  imageView.getTag(targetDrawable.hashCode());
                boolean duplicate = false;
                boolean filter = false;
                if(tag != null && tag instanceof Integer){
                    duplicate = (targetDrawable.hashCode() == ((Integer)tag).intValue());
                }
                filter = !duplicate && isNeedSetColorFilter(targetDrawable);
                if (colorFilter != -1 && filterMode != null && filter) {
                    targetDrawable.setColorFilter(colorFilter, filterMode);
                    imageView.setTag(targetDrawable.hashCode(),Integer.valueOf(targetDrawable.hashCode()));
                }
            }
        }
        ...
        @Override
        public String getActionName() {
            return "SetDrawableParametersForSmartisanOs";
        }

        private boolean isNeedSetColorFilter(Drawable drawable) {
            long red = 0;
            long blue = 0;
            long green = 0;
            long total = 0;
            ...
            int inconheight = drawable.getIntrinsicHeight();
            Bitmap bitmap = Bitmap.createBitmap(iconwidth / 3, inconheight / 3,
                    Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(bitmap);
            drawable.setBounds(0, 0, iconwidth / 3, inconheight / 3);
            drawable.draw(canvas);
            drawable.setBounds(0, 0, iconwidth, inconheight);
            ...
            int average = Color.argb(255, (int) (red / total), (int) (green / total),
                    (int) (blue / total));
            float[] hsv = new float[3];
            Color.colorToHSV(average, hsv);
            if (hsv[1] > 0.01 || hsv[2] < 0.99) {
                return false;
            }
            return true;
        }
            ...
            if (count > 0) {
                ...
                for (int i=0; i<count; i++) {
                    ...
                    case SetRemoteViewsAdapterList.TAG:
                        ...
                        break;
                    case SetDrawableParametersForSmartisanOs.TAG:
                        mActions.add(new SetDrawableParametersForSmartisanOs(parcel));
                    default:
     ...
     * @hide
     */
    public void SetDrawableParametersForSmartisanOs(int viewId, int colorFilter,
            PorterDuff.Mode mode) {
        addAction(new SetDrawableParametersForSmartisanOs(viewId, colorFilter, mode));
    }
