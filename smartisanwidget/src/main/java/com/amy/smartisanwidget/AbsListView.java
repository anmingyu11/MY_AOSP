...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
    ...
    private boolean mReceiverScrolling = false;
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (mReceiverScrolling) {
                return;
            }
            Rect r = new Rect();
            if (getWindowVisibility() == View.VISIBLE && getVisibility() == View.VISIBLE
                    && getGlobalVisibleRect(r)
                    && r.bottom - r.top == getHeight() && r.right - r.left == getWidth()) {
                // Make scroll to top much more quickly if the list is long.
                // TODO: Need A better solution for smooth fast scroll works like FasterScroller.
                // FasterScroller.
                int startPosition = 20;
                int scrollDistance = 1600; // Look and feel tuning
                final View firstChild = getChildAt(0);

                if (firstChild != null) {
                    final int h = firstChild.getHeight();
                    if ( h > 0 ) {
                        startPosition = scrollDistance / h;
                    }
                }

                if (mFirstPosition > startPosition) {
                    setSelection(startPosition);
                }

                mReceiverScrolling = true;
    ...
    /**
     * Track whether last UP/CANCEL touch event got handled in my onTouchEvent().
     * For clean-up over scroll
     */
    private boolean mSavedLastUpHandleState = false;
    ...
    public void setOverScrollMode(int mode) {
        // Never use EdgeEffect, it is too ugly.
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        mSavedLastUpHandleState = false;
        final boolean ret = super.dispatchTouchEvent(ev);
        final int action = ev.getActionMasked();
        if ((!mSavedLastUpHandleState)
                &&(action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL)) {
            // For cleaning up uncompleted over scroll in case UP/CANCEL intercepted by sub-class
            if (mScrollY != 0 && mTouchMode == TOUCH_MODE_OVERSCROLL
                    && (!mSavedLastUpHandleState)) {
                if (mFlingRunnable == null) {
                ...
                // Always spring back when over scroll
                mFlingRunnable.startSpringback();
                setPressed(false);
    ...
    protected void onAttachedToWindow() {
        ...
        filter.addAction(Intent.ACTION_STATUS_BAR_CLICKED);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                //Clear child press state when split touch
                if(mMotionPosition >=0 ){
                    setPressed(false);
                    final View motionView = getChildAt(mMotionPosition - mFirstPosition);
                    if(motionView != null){
    ...
    private boolean isIgnoreMotion(){
        if(this.getViewRootImpl() == null){
            return false;
        }
        return this.getViewRootImpl().mIsIgnoreMoveAction();
    }

    private void onTouchMove(MotionEvent ev) {
        boolean ingoreMovtion = isIgnoreMotion();
        if(ingoreMovtion) return;
        ...
        case TOUCH_MODE_DONE_WAITING:
            ...
            if (child != null) {
                ...
                if (inList && !child.hasFocusable()) {
                    ...
                    if ((mTouchMode == TOUCH_MODE_DOWN || mTouchMode == TOUCH_MODE_TAP) && !ingoreMovtion){
                        removeCallbacks(mTouchMode == TOUCH_MODE_DOWN ?
                                mPendingCheckForTap : mPendingCheckForLongPress);
                        mLayoutMode = LAYOUT_NORMAL;
                        ...
                        return;
                    } else if (!mDataChanged && mAdapter.isEnabled(motionPosition) && !ingoreMovtion) {
                        performClick.run();
                    }
                }
            }
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            if (touchMode != TOUCH_MODE_FLING) {
                if (motionPosition >= 0) {
                    ...
                    mTouchMode = TOUCH_MODE_DOWN;
                    clearScrollingCache();
                } else if (getOverScrollMode() == OVER_SCROLL_ALWAYS) {
                    // If the OverScrollMode is OVER_SCROLL_ALWAYS. We always try
                    // to scroll even user touches outside of actual view
                    mTouchMode = TOUCH_MODE_DOWN;
                }
            }
            mLastY = Integer.MIN_VALUE;
    ...
    private class FlingRunnable implements Runnable {
        ...
        void edgeReached(int delta) {
            ...
            final int overDistance = (int) (vel / (float) mMaximumVelocity * mOverflingDistance);
            mScroller.notifyVerticalEdgeReached(mScrollY, 0, Math.abs(overDistance));
            final int overscrollMode = getOverScrollMode();
            if (overscrollMode == OVER_SCROLL_ALWAYS ||
        ...
        void endFling() {
            boolean resetScrollY = (mTouchMode == TOUCH_MODE_REST || mTouchMode == TOUCH_MODE_FLING);
            mTouchMode = TOUCH_MODE_REST;
            ...
            case TOUCH_MODE_FLING: {
                ...
                if (atEnd) {
                    if (motionView != null) {
                        ...
                        overScrollBy(0, overshoot, 0, 0, 0, 0,
                                0, mOverflingDistance, false);
                        if (mReceiverScrolling) {
                            // do NOT over scroll if fling triggered by clicking on statusbar
                            endFling();
                        }
                    }
                    if (more) {
    ...
    class PositionScroller implements Runnable {
        ...
        void start(final int position) {
            ...
            if (clampedPosition < firstPos || (mReceiverScrolling && firstPos == 0)) {
