...
import android.widget.TextViewUndoRedo.EDIT_TYPE;
...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    /**
     * Set default maxLength for single line TextView
     */
    ...
    private static final int HINT_EXTRA_PADDING = 3;

    // System wide time for last cut or copy action.
    static long LAST_CUT_OR_COPY_TIME;
    // System wide time for last paste.
    static long LAST_PASTE_TIME;
    ...
    private int mMarqueeSpeedLevel =0 ;
    ...
    // However, We don't want 3rd party application to change the drawable of text select handle
    // on Smartisanos. So we just use hardcoded value for the three text select handle.
    ...
    /**
     * Helper class for undo/redo in editable textview.
     */
    ...
    public TextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            case com.android.internal.R.styleable.TextView_textSelectHandleLeft:
                mTextSelectHandleLeftRes = com.android.internal.R.drawable.text_select_handle_left;
                break;

            case com.android.internal.R.styleable.TextView_textSelectHandleRight:
                mTextSelectHandleRightRes = com.android.internal.R.drawable.text_select_handle_right;
                break;

            case com.android.internal.R.styleable.TextView_textSelectHandle:
                mTextSelectHandleRes = com.android.internal.R.drawable.text_select_handle_middle;
                break;
        ...
        if(editable){
            if (!password
                ...
                if (ai != null && (ai.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {
                    mUndoRedo = new TextViewUndoRedo(this);
                }
            }
            if (mEditor.mInputType == EditorInfo.TYPE_NULL) {
                mEditor.mInputType = EditorInfo.TYPE_CLASS_TEXT;
            }
        }
        ...
        if (maxlength == -1 && (singleLine || getMaxLines() == 1)) {
    ...
    public void setInputType(int type) {
        ...
        if (isPassword) {
            setTransformationMethod(PasswordTransformationMethod.getInstance());
            setTypefaceFromAttrs(null /* fontFamily */, SANS, 0);
    ...
    public boolean onPreDraw() {
        ...
        if (this instanceof ExtractEditText && hasSelection() && mEditor != null && getWindowVisibility() == VISIBLE) {
            mEditor.startSelectionActionMode();
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mHint != null && mText.length() == 0) {
            ...
            layout = mHintLayout;
            layout.setHintExtraPadding(isLayoutRtl ? -HINT_EXTRA_PADDING : HINT_EXTRA_PADDING);
        } else {
            layout.setHintExtraPadding(0);
        }
    ...
    protected void makeNewLayout(int wantWidth, int hintWidth,
        ...
        this.makeNewLayout(wantWidth, hintWidth, boring, hintBoring, ellipsisWidth, bringIntoView, true);
    }
    /**
     * The width passed in is now the desired layout width,
     * not the full view width with padding.
    ...
    protected void makeNewLayout(int wantWidth, int hintWidth,
                                 BoringLayout.Metrics boring,
                                 BoringLayout.Metrics hintBoring,
                                 int ellipsisWidth, boolean bringIntoView , boolean stopMarquee) {
        if(stopMarquee){
            stopMarquee();
        }
        ...
        if (mEllipsize == TextUtils.TruncateAt.MARQUEE && stopMarquee) {
    ...
    private static int desired(Layout layout) {
        ...
        for (int i = 0; i < n; i++) {
            // In other place of onMeasure, the des got from Layout.getDesiredWidth()
            // omits trailing paragraph char. Therefore, here should uses getLineMax()
    ...
    public boolean bringPointIntoView(int offset) {
        ...
        // Scroll to make cursor not be clamped
        if (mSingleLine && right - left > hspace && x >= right) {
            int cursorWidth = Math.max(1, (int) FloatMath.ceil(mTextPaint.getStrokeWidth()));
            try {
                final Drawable cd = getResources().getDrawable(mCursorDrawableRes);
                if (cd != null)
                    cursorWidth = cd.getIntrinsicWidth() / 2;
            } catch (NotFoundException e) {
                // mCursorDrawableRes may be 0x0
            }
            right += cursorWidth;
        } else if (!mHorizontallyScrolling && right - left > hspace && right > x) {
            // If cursor has been clamped in multi lines, make sure we don't scroll.
        ...
        if (isFocused()) {
            ...
            // If set MaxLines, the view height and position will not change when
            // line grows over the MaxLines. Therefore, we should not use current
            // line to requestRectangleOnScreen().
            final int maxLine = getMaxLines();
            if (maxLine > 0) {
                line = Math.min(line, maxLine - 1);
            }
            mTempRect.set(x - 2, layout.getLineTop(line), x + 2, layout.getLineTop(line + 1));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mEditor != null) {
            ...
            /*
             * Disallow parent view (like ScrollView) intercepts touch move event
             * to perform cursor moving
             */
            if(action != MotionEvent.ACTION_DOWN) {
                // cancel long press when moving cursor
                if(mEditor.mInterceptTouchEvent) {
                    cancelLongPress();
                    interceptTouch = true;
                }
                ViewParent parent = getParent();
                if(parent != null) {
                    parent.requestDisallowInterceptTouchEvent(mEditor.mInterceptTouchEvent);
                }
            }
        }
        ...
        final boolean touchIsFinished = (action == MotionEvent.ACTION_UP) &&
         ...
         if ((mMovement != null || onCheckIsTextEditor()) && isEnabled()
            ...
            // disable movement when moving cursor
            if (mMovement != null && !interceptTouch) {
            ...
            if (touchIsFinished && mLinksClickable && mAutoLinkMask != 0 && textIsSelectable) {
                ...
                // There is a need to update selection before getting links span, otherwith
                // we may get wrong links if we touch links at different poistions
                int selStart = getSelectionStart();
                int selEnd = getSelectionEnd();
                if (selStart == selEnd) {
                    final int offset = getOffsetForPosition(event.getX(), event.getY());
                    if (offset > 0 && offset != selStart) {
                        Selection.setSelection((Spannable) mText, offset);
                        selStart = offset;
                        selEnd = offset;
                    }
                }
                ClickableSpan[] links = ((Spannable) mText).getSpans(selStart,
    ...
    public void performUndo() {
        if (mUndoRedo != null) {
            boolean hasPerformedUodo = false;
            while(mUndoRedo.getNextUndoType() == EDIT_TYPE.DELETE) {
                mUndoRedo.undo();
                hasPerformedUodo = true;
            }
            if (!hasPerformedUodo) {
                mUndoRedo.undo();
            }
    ...
    public void performRedo() {
        if (mUndoRedo != null) {
            boolean hasPerformedRedo = false;
            while(mUndoRedo.getNextRedoType() == EDIT_TYPE.DELETE) {
                mUndoRedo.redo();
     ...
     * @hide
     */
    public TextViewUndoRedo.EDIT_TYPE getNextUndoType() {
        if (mUndoRedo != null) {
            return mUndoRedo.getNextUndoType();
        }

        return TextViewUndoRedo.EDIT_TYPE.UNKNOW;
    }

    /**
     * @hide
     */
    public TextViewUndoRedo.EDIT_TYPE getNextRedoType() {
        if (mUndoRedo != null) {
            return mUndoRedo.getNextRedoType();
        }
    ...
    public boolean isSuggestionsEnabled() {
        ...
        // disable autocorrection highlight and suggestions when auto link is clickable
        if (mAutoLinkMask != 0 && mLinksClickable) return false;
        ...
        return (variation == EditorInfo.TYPE_TEXT_VARIATION_NORMAL ||
                ...
                variation == EditorInfo.TYPE_TEXT_VARIATION_SHORT_MESSAGE || variation == EditorInfo.TYPE_TEXT_VARIATION_WEB_EDIT_TEXT);
    }
     ...
     * @hide
     */
    public void setCursorControllers(boolean enable) {
        if (mEditor != null) {
            mEditor.mEnableController = enable;
            mEditor.prepareCursorControllers();
        }
    }

    /**
     * Set which action will be hidden in action mode menu
     * @hide
     */
    public void setHiddenContextMenuItem(int flag) {
        mEditor.mHiddenContextMenuItem = flag;
    }

    /**
     * @hide
     */
    public boolean showSelectionController() {
        if (mEditor != null && mEditor.mEnableController && hasSelection()) {
            final Editor.SelectionModifierCursorController controller = mEditor.getSelectionController();
            if (controller != null) {
                mEditor.mHideHandleWhenSelectAll = mEditor.mIsSmartisanUrlInputView;
                controller.show();
                return true;
            }
        }
     ...
     * @hide
     */
    public void setAsSmartisanUrlInputView(boolean enable) {
        if (mEditor != null) {
            mEditor.mIsSmartisanUrlInputView = enable;
        }
    }
    ...
    boolean canUndo() {
        return mUndoRedo != null && mUndoRedo.getCanUndo();
    }

    boolean canRedo() {
        return mUndoRedo != null && mUndoRedo.getCanRedo();
    }

    boolean canInsert() {
        return mText instanceof Editable && getSelectionStart() >= 0 && !hasSelection();
    }
    ...
    private void paste(int min, int max) {
        ...
        if (clip != null) {
            ...
            for (int i=0; i<clip.getItemCount(); i++) {
                LAST_PASTE_TIME = SystemClock.uptimeMillis();
    ...
    public void setVisibility(int visibility) {
        ...
        if (mUndoRedo != null && visibility != View.VISIBLE) {
            mUndoRedo.clearHistory();
        }
    }
    ...
    /**
     * level is from -5 to 5
     * @author mixiaochuan
     * @hide
     */
    public void setMarqueeSpeed(int level){
        if(level < -5 && level > 5){
            mMarqueeSpeedLevel = 0;
            return;
        }
        mMarqueeSpeedLevel = level;
    }
    /**
     * @hide
     * @return
     */
    public int getMarqueeSpeedLevel(){
        return mMarqueeSpeedLevel;
    }
    ...
    private static final class Marquee extends Handler {
        ...
        private int mPixPerSec ;
        private int mResolution ;
        Marquee(TextView v) {
            int speedLevel = v.getMarqueeSpeedLevel();
            mPixPerSec = getPiexPerSecFromLevel(speedLevel);
            mResolution = getResolutionFromLevel(speedLevel);
            final float density = v.getContext().getResources().getDisplayMetrics().density;
            mScrollUnit = (mPixPerSec * density) / mResolution;
            mView = new WeakReference<TextView>(v);
        }
        private int getPiexPerSecFromLevel(int level){
            return MARQUEE_PIXELS_PER_SECOND + (level * 3);
        }
        private int getResolutionFromLevel(int level){
            int v  = MARQUEE_PIXELS_PER_SECOND + (level * 3);
            return 1000/v ;
        }
        @Override
        ...
        void tick() {
            ...
            if (textView != null && (textView.isFocused() || textView.isSelected())) {
                ...
                if (mScroll > mMaxScroll) {
                    ...
                    sendEmptyMessageDelayed(MESSAGE_TICK, mResolution);
                }
                textView.invalidate();
            }
        }
