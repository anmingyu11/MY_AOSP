...
public final class WindowManagerGlobal {
    ...
    public boolean isWindowInThumbMode(){
        if(sWindowSession == null) return false;

        try {
            return sWindowSession.getInThumbMode();
        } catch (RemoteException e) {
            e.printStackTrace();
            return false;
        }
    ...
    public void resetWindowOneHandedState(){
        if(sWindowSession == null) return;

        try {
            sWindowSession.resetWindowStateFromThumbMode();
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
    ...
    public void updateViewLayout(View view, ViewGroup.LayoutParams params) {
        ...
        synchronized (mLock) {
            ...
            try {
                ...
                if (DEBUG)
                    Log.e(TAG, "Target view not found");
                index = -1;
            }
            if (index != -1) {
     ...
     * @hide
     */
    public static boolean needAddAppStartingWindow(String pkgName) {
        if(pkgName == null)
            return false;
        String[] whiteList = {
                "com.taobao",
