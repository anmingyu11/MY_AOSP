...
public class Surface implements Parcelable {
    ...
    //smartisanOS, add 3X3matrix
    /**
     * Exception thrown when caputure a surface but it not refreshed content
     * @hide
     */
    public static class NoNewContentException extends Exception {
        public NoNewContentException() {
        }

        public NoNewContentException(String name) {
            super(name);
        }
    }
    /** @hide */
    public static class AcquireSurfaceInfo {
        //0: return bitmap with locked bits directly
        //default: yes
        public static final int DIRECT = 0;
        //1: return a new bitmap copyed locked bits, needs no release
        public static final int COPY = 1;
        public Rect dirtyRect;
        public int textId;
        //checking surface content change for continuous capture
        //to avoid unnecessary reload for user
        //capture point is indentified by dequeuecount, passed by
        //BufferQueue, user can save last one for compare

        //BEAWARED it is unsign int 64 in native,
        //so don't change, compare it in Java, it maybe wrong after bigger enough
        public long lastFrameCount;
        public long frameCount;//updated every capture in native method
        public int  acquireMode;
        public boolean forceCapture;
        public void reset() {
            if (null != dirtyRect) dirtyRect.setEmpty();
            textId = -1;
            lastFrameCount = 0;
            frameCount = 0;
            forceCapture = false;
        ...
        public AcquireSurfaceInfo() {
            reset();
        }
        public AcquireSurfaceInfo(boolean force, int mode) {
            reset();
            forceCapture = force;
            acquireMode  = mode;
        }
    };
    //in future, if bind texture inside function, then
    //1. must in gl thread,
    //2. there is max number of surface limitation, currently 32 / number of local buffers
    //todo improve, because windowMananger will capture a lot
    /** @hide */
    public Bitmap acquireSurface(AcquireSurfaceInfo acquireInfo) throws NoNewContentException{
        return nativeAcquireSurface(mNativeObject, acquireInfo);
    }
    /** @hide */
    public void releaseSurface() {
        nativeReleaseSurface(mNativeObject);
    }
    private native Bitmap nativeAcquireSurface(int nativeObject, AcquireSurfaceInfo acquireInfo) throws NoNewContentException;
    private native void nativeReleaseSurface(int nativeObject);
}
