...
public final class ViewRootImpl implements ViewParent,
    ...
    private static float THUMB_OFFSET;
    private static boolean mThumbDateInited;
    boolean isInThumbModeBeforeTouchDown = false;

    boolean mDrawDuringAnimation;
    ...
    public static final int BIT_THUMB_MODE_ENABLE = 1<<0;
    public static final int BIT_WINDOW_IN_THUMB_MODE = 1<<1;
    public static final int TRANSACTION_THUMB_STATES = 1000;
    public static final String TOKEN_TRANSACTION_THUMB_STATES= "android.view.IWindow.DISPATCH_THUMB_STATES";
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        initThumbConfig(context);
    }

    private void initThumbConfig(Context context){
        if(!mThumbDateInited){
            try{
                THUMB_OFFSET = mWindowSession.getThumbOffset();
            } catch (RemoteException re) {
            }
            mThumbDateInited = true;
        }
    }
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView) {
        synchronized (this) {
            if (mView == null) {
                mView = view;
                mDrawDuringAnimation = attrs.drawDuringAnimation;
    ...
    void updateThumbModeState(int states) {
        ...
        if((mThumbModeStates & BIT_WINDOW_IN_THUMB_MODE) == 0){
    ...
    void returnNormalModeFromThumb(boolean isInThumbModeBeforeTouchDown){
        mHandler.removeCallbacks(mResetThumbModeRunnable);
        mHandler.postDelayed(mResetThumbModeRunnable,
                isInThumbModeBeforeTouchDown ? DELAY_RESET_THUMB_STATE_TOUCH_TIMEOUT : DELAY_RESET_THUMB_STATE_IDLE_TIMEOUT);
    }
    ...
    private void performTraversals() {
        ...
        if (didLayout || newSurface) {
            ...
            if ((host.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) != 0) {
                ...
                // Following codes fixed bug 0009958 in 3.11 sfo daily.
                // But also causes a new bug 0014430 in 3.29 daily.
                // After remove those codes in 3.29 daily, 0009958 can not be reproduced any more.
                // So there must be some fix of native fix in native surface about TransparentRegion those days.
                // And in smartisan rom in 9100/find5/xiaomi, there is not any above issues at all !
                /*try {
                    // This is a backup step
                    // Make sure that when in thumb mode, transparent area of a window also have a thumb offset.
                    if(mWindowSession.getInThumbMode()){
                        Rect rect = mTransparentRegion.getBounds();
                        int thumbOffset = mWindowSession.getThumbOffset();
                        mWindowSession.setTransparentRegion(mWindow,
                                new Region(rect.left, rect.top+thumbOffset, rect.right, rect.bottom+thumbOffset));
                    }
                } catch (RemoteException e) {
                }*/
            }
    ...
    private final static int MSG_DISPATCH_THUMB_MODE_STATES = 26;
            ...
            if (mLastWasImTarget && !isInLocalFocusMode()) {
                ...
                if (imm != null) {
                    ...
                    if (result == InputMethodManager.DISPATCH_HANDLED) {
                        ...
                        // There maybe crash of input method and causing the IMM's mCurMethod or
                        // mInputChannel is null. We return FORWARD instead of FINISH_NOT_HANDLED
                        // to avoid keyevent failing to send to view.
                        Log.w(TAG, "Some error happens in InputMethodManager, "
                                + "and get a DISPATCH_NOT_HANDLED, return FORWARD");
                        return FORWARD;
                    } else {
    ...
    private void deliverInputEvent(QueuedInputEvent q) {
        ...
        try {
            ...
            if(null != mViewInterceptInputStage && mViewInterceptInputStage.deliverToTarget(q)) {
                return;
            }
    ...
    private void finishInputEvent(QueuedInputEvent q) {
        if (q.mEvent instanceof MotionEvent) {
            final MotionEvent event = (MotionEvent) q.mEvent;
            if (event.isTouchEvent() && (event.getAction() == MotionEvent.ACTION_DOWN)) {
                isInThumbModeBeforeTouchDown = ((mThumbModeStates & BIT_WINDOW_IN_THUMB_MODE) != 0);
            }
            if ((mThumbModeStates & BIT_WINDOW_IN_THUMB_MODE) != 0) {
                boolean isTouchEvent = event.isTouchEvent();
                boolean isActionUpOrCancel = (event.getAction() == MotionEvent.ACTION_UP)
                                        || (event.getAction() == MotionEvent.ACTION_CANCEL);
                if(isTouchEvent && isActionUpOrCancel){
                    returnNormalModeFromThumb(isInThumbModeBeforeTouchDown);
                }
            }
        }
    ...
    private boolean mIgnoreMoveAction = false;
    private boolean isLastTouchDownNotInThumbMode = false;
    public boolean mIsIgnoreMoveAction(){
        return mIgnoreMoveAction;
    }
    ...
    final class WindowInputEventReceiver extends InputEventReceiver {
        ...
        /**
         * Thumb mode touch event process logic
         * When an movtion event move from a app window to  the wallpaper window,
         * this is a situation android not handle, we shoud always pass the action up to the app window,
         * or app will lost a action up event, this lead to an unusal situation.
         **/

        @Override
        public void onInputEvent(InputEvent event) {
            if ((mThumbModeStates & BIT_THUMB_MODE_ENABLE) != 0) {
                // Do not intercept input touch event from launcher at any
                // time.
                if (event instanceof MotionEvent) {
                    boolean inThumbMode = ((mThumbModeStates & BIT_WINDOW_IN_THUMB_MODE) != 0);
                    final MotionEvent motionEvent = (MotionEvent) event;

                    if(!inThumbMode && motionEvent.getAction() == MotionEvent.ACTION_DOWN){
                        isLastTouchDownNotInThumbMode = true;
                        mIgnoreMoveAction = false;
                    }else if(inThumbMode && motionEvent.getAction() == MotionEvent.ACTION_DOWN){
                        isLastTouchDownNotInThumbMode = false;
                        mIgnoreMoveAction = false;
                        mHandler.removeCallbacks(mResetThumbModeRunnable);
                    }else if (motionEvent.getAction() == MotionEvent.ACTION_UP){

                    }else if(motionEvent.getAction() == MotionEvent.ACTION_MOVE && isLastTouchDownNotInThumbMode
                            && inThumbMode){
                        mIgnoreMoveAction = true;
                    }

                    //ignore low touch when in thumb mode
                    if (inThumbMode) {
                        if (motionEvent.getRawY() < THUMB_OFFSET &&
                        // Do not finish ACTION_UP, ACTION_UP must be
                        // send to the app window
    ...
    //SmartisanOS Ext
    final class ViewInterceptInputStage extends InputStage {
        private View mTarget;
        private boolean mOneshot;
        ViewInterceptInputStage(InputStage next, View target, boolean oneshot) {
            super(next);
            mTarget = target;
            mOneshot = oneshot;
        }
        //if intercept target handled event, then stop directly here
        public boolean deliverToTarget(QueuedInputEvent q) {
            final int source = q.mEvent.getSource();
            boolean handled = false;
            if (q.mEvent instanceof MotionEvent
                ...
                final MotionEvent event = (MotionEvent)q.mEvent;
                final int action = event.getAction();
                if (mOneshot && MotionEvent.ACTION_DOWN == action) {
                    //just for safe, ensure oneshot interceptor whill remove
                    removeInputInerceptor(mTarget);
                } else {
                    handleDispatchDoneAnimating();
                    handled = mTarget.dispatchPointerEvent(event);
                }
                if (mOneshot && MotionEvent.ACTION_UP == action) {
                    removeInputInerceptor(mTarget);
                }
            }
            if (handled) {
        ...
        boolean isTarget(View target) {
            return mTarget == target;
        }
        boolean isOneshot() {
            return mOneshot;
        }
        void setTarget(View target, boolean oneshot) {
            mTarget = target;
            mOneshot = oneshot;
        }
    }
    ViewInterceptInputStage mViewInterceptInputStage;
    ViewInterceptInputStage mTmpViewInterceptInputStage;
    boolean setInputInerceptor(View target, boolean oneshot) {
        if (null == target) {
            return false;
        }
        if (null == mTmpViewInterceptInputStage) {
            mTmpViewInterceptInputStage = new ViewInterceptInputStage(mFirstInputStage, target, oneshot);
        } else {
            mTmpViewInterceptInputStage.setTarget(target, oneshot);
        }
        mViewInterceptInputStage = mTmpViewInterceptInputStage;
        return true;
    }
    boolean removeInputInerceptor(View target) {
        if (null != mViewInterceptInputStage
            && mViewInterceptInputStage.isTarget(target) ) {
            mViewInterceptInputStage.setTarget(null, true);
            mViewInterceptInputStage = null;
            return true;
        }
        return false;
    }
