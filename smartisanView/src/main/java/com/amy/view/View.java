...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
    ...
    /**
     * @hide
     * to show current window is smartisan launcher
     */
    public static final int SYSTEM_UI_FLAG_SMARTISAN_LAUNCHER = 0x00002000;

    /**
     * @hide
     * to show current window is keygaurd
     */
    public static final int SYSTEM_UI_FLAG_SMARTISAN_KEYGUARD = 0x00004000;

    /**
     * @hide
     * o show current window is android launcher
     */
    public static final int SYSTEM_UI_FLAG_ANDROID_LAUNCHER = 0x00008000;
   ...
   /**
    ...
    public ViewParent findRootParent() {
        ViewParent lastParent = mParent;
        if (null != lastParent) {
            ViewParent parent;
            while(null != (parent = lastParent.getParent())){
                lastParent = parent;
            }
        }
        return lastParent;
     ...
     * @hide
     */
    public boolean setInputInerceptor(View target, boolean oneshot) {
        ViewParent root = findRootParent();
        if (root.getClass() == ViewRootImpl.class) {
            return ((ViewRootImpl)root).setInputInerceptor(target, oneshot);
        } else {
            Log.w(VIEW_LOG_TAG, "setInputInerceptor but root is not ViewRoot:"+root);
            return false;
        }
    }
    /**
     * @hide
     */
    public boolean removeInputInterceptor(View target) {
        ViewParent root = findRootParent();
        if (root.getClass() == ViewRootImpl.class) {
            return ((ViewRootImpl)root).removeInputInerceptor(target);
        } else {
            Log.w(VIEW_LOG_TAG, "setInputInerceptor but root is not ViewRoot:"+root);
            return false;
        }
