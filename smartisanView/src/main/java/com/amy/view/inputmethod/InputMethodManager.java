...
public final class InputMethodManager {
    ...
    static final int MSG_SET_HAS_BEEN_INACTIVE = 8;
    ...
    class H extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SET_HAS_BEEN_INACTIVE: {
                    synchronized (mH) {
                        mHasBeenInactive = true;
                    }
                    return;
    ...
    final IInputMethodClient.Stub mClient = new IInputMethodClient.Stub() {
         ...
         */
        @Override
        public void setHasBeenInactive() {
            mH.sendMessage(mH.obtainMessage(MSG_SET_HAS_BEEN_INACTIVE, 0, 0));
        }
    };
     ...
     * @hide
     */
     //finish input on view detach window will cause showinput failed in the
     //case view is detatched but other view in the same window show input
     //method, change to save last served view's token and check save token
     //in case of served view's token is null(detached view)
    public void windowDismissed(IBinder appWindowToken) {
        checkFocus();
        synchronized (mH) {
            final IBinder servedWinToken = null == mServedView ? null : mServedView.getWindowToken();
            boolean tokenMatch = (servedWinToken == appWindowToken)
              || (null == servedWinToken && mLastServedWinToken == appWindowToken);
            if (mServedView != null && tokenMatch) {
                finishInputLocked();
            }
        }
    }
    ...
    void focusInLocked(View view) {
        if (DEBUG) Log.v(TAG, "focusIn: "+view+", mCurRootView="+mCurRootView
            +", view.getRootView="+view.getRootView());
    ...
    private boolean checkFocusNoStartInput(boolean forceNewFocus, boolean finishComposingText) {
        ...
        synchronized (mH) {
            ...
            mLastServedWinToken = mServedView.getWindowToken();
            mCurrentTextBoxAttribute = null;
            mCompletions = null;
            mServedConnecting = true;
        }
    ...
    //SmartisanOS EXT
    //solved issue: mServicedView detached from window can not finishInput in window dissmissed
    private IBinder mLastServedWinToken;
}
